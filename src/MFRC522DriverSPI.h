...
public:
  ...
  MFRC522DriverSPI(
      MFRC522DriverPin &chipSelectPin, SPIClass &spiClass = SPI,
      const SPISettings spiSettings = SPISettings(4000000u /* 4MHz */, MSBFIRST, SPI_MODE0)
  )
      : MFRC522Driver(), _chipSelectPin(chipSelectPin), _spiClass(spiClass), _spiSettings(spiSettings) {};

  MFRC522DriverSPI(
      MFRC522DriverPin &chipSelectPin, int8_t sck = -1, int8_t miso = -1, int8_t mosi = -1,
      SPIClass &spiClass = SPI,
      const SPISettings spiSettings = SPISettings(4000000u /* 4MHz */, MSBFIRST, SPI_MODE0)
  )
      : MFRC522Driver(), _chipSelectPin(chipSelectPin), _sck(sck), _miso(miso), _mosi(mosi),
        _spiClass(spiClass), _spiSettings(spiSettings) {};

protected:
  // Pins.
  // byte _chipSelectPin;        // Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS,
  // active low)
  MFRC522DriverPin
      &_chipSelectPin; // Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)

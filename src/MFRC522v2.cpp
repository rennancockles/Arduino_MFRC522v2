...
/////////////////////////////////////////////////////////////////////////////////////
void MFRC522::SetDriver(MFRC522Driver &driver) { _driver = &driver; };
...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(
  ...
  for (uint16_t i = 5000; i > 0; i--) {
    ...
    if (n & 0x04) { // CRCIRq bit set - calculation done
      _driver->PCD_WriteRegister(
          PCD_Register::CommandReg, PCD_Command::PCD_Idle
...
bool MFRC522::PCD_Init() {
  ...
  if (!_driver || _driver->init() == false) { return false; }
  ...
  //      digitalWrite(_driver->_resetPowerDownPin, HIGH);    // Exit power down mode. This triggers a hard
  //      reset.
  //      // Section 8.8.2 in the datasheet says the oscillator start-up time is the start up time of the
  //      crystal + 37,74μs. Let us be generous: 50ms. delay(50); hardReset = true;
  //    }
  //  }
  ...
  _driver->PCD_WriteRegister(
      PCD_Register::TModeReg, 0x80
  ); // TAuto=1; timer starts automatically at the end of the transmission in all communication modes at all
     // speeds
  _driver->PCD_WriteRegister(
      PCD_Register::TPrescalerReg, 0xA9
  ); // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25μs.
  _driver->PCD_WriteRegister(
      PCD_Register::TReloadRegH, 0x03
  ); // Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
  _driver->PCD_WriteRegister(PCD_Register::TReloadRegL, 0xE8);

  _driver->PCD_WriteRegister(
      PCD_Register::TxASKReg, 0x40
  ); // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
  _driver->PCD_WriteRegister(
      PCD_Register::ModeReg, 0x3D
  ); // Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO
     // 14443-3 part 6.2.4)
...
void MFRC522::PCD_Reset() {
  _driver->PCD_WriteRegister(
      PCD_Register::CommandReg, PCD_Command::PCD_SoftReset
  ...
  do {
    ...
    delay(50);
  } while ((_driver->PCD_ReadRegister(PCD_Register::CommandReg) & (1 << 4)) &&
           ((++countTries) < 3 /* Timeout after 3 tries. */));
} // End PCD_Reset()
...
void MFRC522::PCD_AntennaOn() {
  ...
  if ((value & 0x03) != 0x03) { _driver->PCD_WriteRegister(PCD_Register::TxControlReg, value | 0x03); }
} // End PCD_AntennaOn()
...
MFRC522::PCD_Version MFRC522::PCD_GetVersion() {
  ...
  switch (version) {
    case 0x12: return PCD_Version::Version_Counterfeit;
    case 0xb2: return PCD_Version::Version_FM17522_1;
    case 0x88: return PCD_Version::Version_FM17522;
    case 0x89: return PCD_Version::Version_FM17522E;
    case 0x90: return PCD_Version::Version_0_0;
    case 0x91: return PCD_Version::Version_1_0;
    case 0x92: return PCD_Version::Version_2_0;
    default: return PCD_Version::Version_Unknown;
  }
} // End PCD_GetVersion()
...
bool MFRC522::PCD_PerformSelfTest() {
  ...
  for (uint8_t i = 0; i < 64; i++) {
    if (result[i] != pgm_read_byte(&(reference[i]))) { verified = false; }
  }
...
MFRC522::StatusCode MFRC522::PCD_TransceiveData(
    ...
    byte *validBits, ///< In/Out: The number of valid bits in the last byte. 0 for 8 valid bits. Default
                     ///< nullptr.
    byte rxAlign,    ///< In: Defines the bit position in backData[0] for the first bit received. Default 0.
    bool checkCRC    ///< In: True => The last two bytes of the response is assumed to be a CRC_A that must be
                     ///< validated.
) {
  byte waitIRq = 0x30; // RxIRq and IdleIRq
  return PCD_CommunicateWithPICC(
      PCD_Command::PCD_Transceive, waitIRq, sendData, sendLen, backData, backLen, validBits, rxAlign, checkCRC
  );
} // End PCD_TransceiveData()

/**
 * Transfers data to the MFRC522 FIFO, executes a command, waits for completion and transfers data back from
 * the FIFO. CRC validation can only be done if backData and backLen are specified.
 *
 * @return StatusCode::STATUS_OK on success, StatusCode::STATUS_??? otherwise.
 */
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(
  ...
  // In PCD_Init() we set the TAuto flag in TModeReg. This means the timer automatically starts when the PCD
  // stops transmitting. Each iteration of the do-while-loop takes 17.86μs.
  // TODO check/modify for other architectures than Arduino Uno 16bit
  uint16_t i;
  for (i = 2000; i > 0; i--) {
    byte n = _driver->PCD_ReadRegister(
        PCD_Register::ComIrqReg
  ...
  if (i == 0) { return StatusCode::STATUS_TIMEOUT; }

  // Stop now if any errors except collisions were detected.
  byte errorRegValue = _driver->PCD_ReadRegister(
      PCD_Register::ErrorReg
  ...
  if (backData && backLen) {
    ...
    if (n > *backLen) { return StatusCode::STATUS_NO_ROOM; }
    *backLen = n;                                                               // Number of bytes returned
    _driver->PCD_ReadRegister(PCD_Register::FIFODataReg, n, backData, rxAlign); // Get received data from FIFO
    _validBits = _driver->PCD_ReadRegister(PCD_Register::ControlReg) &
                 0x07; // RxLastBits[2:0] indicates the number of valid bits in the last received byte. If
  ...
  if (backData && backLen && checkCRC) {
    ...
    if (*backLen == 1 && _validBits == 4) { return StatusCode::STATUS_MIFARE_NACK; }
    // We need at least the CRC_A value and all 8 bits of the last byte must be received.
    if (*backLen < 2 || _validBits != 0) { return StatusCode::STATUS_CRC_WRONG; }
    // Verify CRC_A - do our own calculation and store the control in controlBuffer.
    byte controlBuffer[2];
    MFRC522::StatusCode status = PCD_CalculateCRC(&backData[0], *backLen - 2, &controlBuffer[0]);
    if (status != StatusCode::STATUS_OK) { return status; }
 ...
 * anticollision or selection. 7 bit frame. Beware: When two PICCs are in the field at the same time I often
 * get StatusCode::STATUS_TIMEOUT - probably due do bad antenna design.
 *
 * @return StatusCode::STATUS_OK on success, StatusCode::STATUS_??? otherwise.
 */
 ...
 * Beware: When two PICCs are in the field at the same time I often get StatusCode::STATUS_TIMEOUT - probably
 * due do bad antenna design.
 *
 * @return StatusCode::STATUS_OK on success, StatusCode::STATUS_??? otherwise.
 */
MFRC522::StatusCode MFRC522::PICC_REQA_or_WUPA(
    byte command,     ///< The command to send - PICC_Command::PICC_CMD_REQA or PICC_Command::PICC_CMD_WUPA
    byte *bufferATQA, ///< The buffer to store the ATQA (Answer to request) in
    byte *bufferSize  ///< Buffer size, at least two bytes. Also number of bytes returned if
                      ///< StatusCode::STATUS_OK.
) {
  ...
  PCD_ClearRegisterBitMask(
      PCD_Register::CollReg, 0x80
  );             // ValuesAfterColl=1 => Bits received after collision are cleared.
  validBits = 7; // For REQA and WUPA we need the short frame format - transmit only 7 bits of the last (and
                 // only) byte. TxLastBits = BitFramingReg[2..0]
 ...
 * 		- The chosen PICC is in state ACTIVE(*) and all other PICCs have returned to state IDLE/HALT. (Figure 7
 * of the ISO/IEC 14443-3 draft.)
...
MFRC522::StatusCode MFRC522::PICC_Select(
  ...
  //		Byte 0: SEL 				Indicates the Cascade Level: PICC_Command::PICC_CMD_SEL_CL1,
  // PICC_Command::PICC_CMD_SEL_CL2 or PICC_Command::PICC_CMD_SEL_CL3 		Byte 1: NVB					Number of Valid
  // Bits (in complete command, not just the UID): High nibble: complete bytes, Low nibble: Extra bits. Byte
  // 2: UID-data or CT		See explanation below. CT means Cascade Tag. 		Byte 3: UID-data 		Byte 4: UID-data
  // Byte 5: UID-data 		Byte 6: BCC					Block Check Character - XOR of bytes 2-5 		Byte 7: CRC_A Byte
  // 8: CRC_A
  ...
  // Sanity checks
  if (validBits > 80) { return StatusCode::STATUS_INVALID; }
  ...
  while (!uidComplete) {
    ...
    switch (cascadeLevel) {
      ...
      default: return StatusCode::STATUS_INTERNAL_ERROR; break;
    }
    ...
    if (useCascadeTag) { buffer[index++] = PICC_Command::PICC_CMD_CT; }
    ...
    // Repeat anti collision loop until we can transmit all UID bits + BCC and receive a SAK - max 32
    // iterations.
    selectDone = false;
    while (!selectDone) {
      ...
      if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level are known. This is a SELECT.
        ...
        if (result != StatusCode::STATUS_OK) { return result; }
        ...
        // Serial.print(F("ANTICOLLISION: currentLevelKnownBits=")); Serial.println(currentLevelKnownBits,
        // DEC);
      ...
      // Set bit adjustments
      rxAlign =
          txLastBits; // Having a separate variable is overkill. But it makes the next line easier to read.
      _driver->PCD_WriteRegister(
          PCD_Register::BitFramingReg, (rxAlign << 4) + txLastBits
      ...
      if (result == StatusCode::STATUS_COLLISION) { // More than one PICC in the field => collision.
        byte valueOfCollReg = _driver->PCD_ReadRegister(
    ...
    // Verify CRC_A - do our own calculation and store the control in buffer[2..3] - those bytes are not
    // needed anymore.
...
MFRC522::StatusCode MFRC522::PICC_HaltA() {
  ...
  if (result == StatusCode::STATUS_TIMEOUT) { return StatusCode::STATUS_OK; }
 ...
 * This command manages MIFARE authentication to enable a secure communication to any MIFARE Mini, MIFARE 1K
 * and MIFARE 4K card. The authentication is described in the MFRC522 datasheet section 10.3.1.9 and
 * http://www.nxp.com/documents/data_sheet/MF1S503x.pdf section 10.1. For use with MIFARE Classic PICCs. The
 * PICC must be selected - ie in state ACTIVE(*) - before calling this function. Remember to call
 * PCD_StopCrypto1() after communicating with the authenticated PICC - otherwise no new communications can
 * start.
 *
 * All keys are set to FFFFFFFFFFFFh at chip delivery.
 *
 * @return StatusCode::STATUS_OK on success, StatusCode::STATUS_??? otherwise. Probably
 * StatusCode::STATUS_TIMEOUT if you supply the wrong key.
 */
MFRC522::StatusCode MFRC522::PCD_Authenticate(
    byte command,    ///< PICC_Command::PICC_CMD_MF_AUTH_KEY_A or PICC_Command::PICC_CMD_MF_AUTH_KEY_B
 ...
 * The MF0ICU1 responds to the READ command by sending 16 bytes starting from the page address defined by the
 * command argument. For example; if blockAddr is 03h then pages 03h, 04h, 05h, 06h are returned. A roll-back
 * is implemented: If blockAddr is 0Eh, then the contents of pages 0Eh, 0Fh, 00h and 01h are returned.
 *
...
MFRC522::StatusCode MFRC522::MIFARE_Read(
    byte blockAddr,  ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to
                     ///< return data from.
    byte *buffer,    ///< The buffer to store the data in.
    byte *bufferSize ///< Buffer size, at least 18 bytes. Also number of bytes returned if
                     ///< StatusCode::STATUS_OK.
) {
  MFRC522::StatusCode result;

  // Sanity check
  if (buffer == nullptr || *bufferSize < 18) { return StatusCode::STATUS_NO_ROOM; }
 ...
 * Even though 16 bytes are transferred to the Ultralight PICC, only the least significant 4 bytes (bytes 0 to
 * 3) are written to the specified address. It is recommended to set the remaining bytes 04h to 0Fh to all
 * logic 0.
 * *
 * @return StatusCode::STATUS_OK on success, StatusCode::STATUS_??? otherwise.
 */
MFRC522::StatusCode MFRC522::MIFARE_Write(
    byte blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to
                    ///< write to.
  ...
  // Sanity check
  if (buffer == nullptr || bufferSize < 16) { return StatusCode::STATUS_INVALID; }
...
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_Write(
  ...
  // Sanity check
  if (buffer == nullptr || bufferSize < 4) { return StatusCode::STATUS_INVALID; }
...
/**
 * MIFARE Decrement subtracts the delta from the value of the addressed block, and stores the result in a
 * volatile memory. For MIFARE Classic only. The sector containing the block must be authenticated before
 * calling this function. Only for blocks in "value block" mode, ie with access bits [C1 C2 C3] = [110] or
...
/**
 * MIFARE Increment adds the delta to the value of the addressed block, and stores the result in a volatile
 * memory. For MIFARE Classic only. The sector containing the block must be authenticated before calling this
 * function. Only for blocks in "value block" mode, ie with access bits [C1 C2 C3] = [110] or [001]. Use
...
MFRC522::StatusCode MFRC522::MIFARE_Restore(
  ...
  // The datasheet describes Restore as a two step operation, but does not explain what data to transfer in
  // step 2. Doing only a single step does not work, so I chose to transfer 0L in step two.
...
MFRC522::StatusCode MFRC522::PCD_NTAG216_AUTH(const byte password[4], byte pACK[]) {
  ...
  for (byte i = 0; i < 4; i++) { cmdBuffer[i + 1] = password[i]; }
  ...
  result = PCD_CommunicateWithPICC(
      PCD_Command::PCD_Transceive, waitIRq, cmdBuffer, 7, cmdBuffer, &rxlength, &validBits
  );
...
MFRC522::StatusCode MFRC522::PCD_MIFARE_Transceive(
  ...
  // Sanity check
  if (sendData == nullptr || sendLen > 16) { return StatusCode::STATUS_INVALID; }
  ...
  byte validBits = 0;
  result = PCD_CommunicateWithPICC(
      PCD_Command::PCD_Transceive, waitIRq, cmdBuffer, sendLen, cmdBuffer, &cmdBufferSize, &validBits
  );
  if (acceptTimeout && result == StatusCode::STATUS_TIMEOUT) { return StatusCode::STATUS_OK; }
  if (result != StatusCode::STATUS_OK) { return result; }
  // The PICC must reply with a 4 bit ACK
  if (cmdBufferSize != 1 || validBits != 4) { return StatusCode::STATUS_ERROR; }
  if (cmdBuffer[0] != MIFARE_Misc::MF_ACK) { return StatusCode::STATUS_MIFARE_NACK; }
  return StatusCode::STATUS_OK;
...
const __FlashStringHelper *MFRC522::PICC_GetTypeName(
  ...
  switch (piccType) {
    case PICC_Type::PICC_TYPE_ISO_14443_4: return F("PICC compliant with ISO/IEC 14443-4"); break;
    case PICC_Type::PICC_TYPE_ISO_18092: return F("PICC compliant with ISO/IEC 18092 (NFC)"); break;
    case PICC_Type::PICC_TYPE_MIFARE_MINI: return F("MIFARE Mini, 320 bytes"); break;
    case PICC_Type::PICC_TYPE_MIFARE_1K: return F("MIFARE 1KB"); break;
    case PICC_Type::PICC_TYPE_MIFARE_4K: return F("MIFARE 4KB"); break;
    case PICC_Type::PICC_TYPE_MIFARE_UL: return F("MIFARE Ultralight or Ultralight C"); break;
    case PICC_Type::PICC_TYPE_MIFARE_PLUS: return F("MIFARE Plus"); break;
    case PICC_Type::PICC_TYPE_TNP3XXX: return F("MIFARE TNP3XXX"); break;
    case PICC_Type::PICC_TYPE_NOT_COMPLETE: return F("SAK indicates UID is not complete."); break;
...
/**
 * Calculates the bit pattern needed for the specified access bits. In the [C1 C2 C3] tuples C1 is MSB (=4)
 * and C3 is LSB (=1).
 */
void MFRC522::MIFARE_CalculateAccessBits(
    byte accessBitBuffer[3], ///< Pointer to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
    const byte
    ...
    const byte g3 ///< Access bits C1 C2 C3] for the sector trailer, block 3 (for sectors 0-31) or block 15
                  ///< (for sectors 32-39)
) const {

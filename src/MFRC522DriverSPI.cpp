...
bool MFRC522DriverSPI::init() {
  ...
  if (_sck != -1 && _miso != -1 && _mosi != -1) {
    _spiClass.begin(_sck, _miso, _mosi);
  } else {
    _spiClass.begin();
  }
 ...
 */
void MFRC522DriverSPI::PCD_WriteRegister(
    const PCD_Register reg, ///< The register to write to. One of the PCD_Register enums.
...
void MFRC522DriverSPI::PCD_WriteRegister(
  ...
  for (byte index = 0; index < count; index++) { _spiClass.transfer(values[index]); }
  _chipSelectPin.high();      // Release slave again
  _spiClass.endTransaction(); // Stop using the SPI bus
} // End PCD_WriteRegister()
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
byte MFRC522DriverSPI::PCD_ReadRegister(
    const PCD_Register reg ///< The register to read from. One of the PCD_Register enums.
) {
  byte value;
  _spiClass.beginTransaction(_spiSettings); // Set the settings to work with SPI bus
  _chipSelectPin.low();                     // Select slave
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void MFRC522DriverSPI::PCD_ReadRegister(
    const PCD_Register reg, ///< The register to read from. One of the PCD_Register enums.
  ...
  byte address =
      (byte)0x80 |
      (reg << 1); // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
  ...
  while (index < count - 1) {
    values[index] =
        _spiClass.transfer(address); // Read value and tell that we want to read the same address again.
    index++;
  }
